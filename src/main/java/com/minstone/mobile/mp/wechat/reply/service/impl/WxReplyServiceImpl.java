...
//    4-8.物理删除关键词
    ...
    public List<WxReply> getInfo(String publicCode, Integer replyType) throws WxErrorException {
        // 检查公众号是否存在
        String[] publiCodes = {publicCode};
        List<String> checkPublicCodes = wxPublicDao.selectPublicCodes(publiCodes);
        if (checkPublicCodes == null) {
     ...
     */
    @Override
    public boolean forceDeleteKeyword(String keywordCode) throws WxErrorException, CommonException {
        if (wxReplyKeywordDao.forceDeleteByPrimaryKey(keywordCode) < 1) {
    ...
    public String addReplyRule(WxReplyRule replyRule) throws WxErrorException, CommonException {
        ...
        // 查询公众号是否存在
        String[] publiCodes = {replyRule.getPublicCode()};
        List<String> checkPublicCodes = wxPublicDao.selectPublicCodes(publiCodes);
        if (checkPublicCodes == null) {
            throw new CommonException(404, "该公众号不存在");
        }
...
//            return false;
     ...
     */
    @Override
    public boolean forceDeleteRule(String ruleCode) throws WxErrorException, CommonException {
        if (wxReplyKeywordDao.forceDeleteByRuleCode(ruleCode) < 1) {
//            throw new CommonException(500,"服务器异常，删除关键词出错");
            logger.error("（物理）删除关键词出错");
            return false;
        } else if (wxReplyRuleDao.forceDeleteByPrimaryKey(ruleCode) < 1) { // 删除规则
//            throw new CommonException(500,"服务器异常，删除关键词规则出错");
     ...
     * @throws WxErrorException
     */
    public boolean forceDeleteRuleBatch(String[] ruleCodes) throws WxErrorException, CommonException {
        if (wxReplyRuleDao.forceDeleteBatch(ruleCodes) < 1) {
            logger.error("（物理）批量删除关键词规则出错");
            return false;
        } else if (wxReplyKeywordDao.forceDeleteByRuleCodeBatch(ruleCodes) < 1) {
            logger.error("（物理）批量删除关键词规则出错");
            return false;
        } else {
